...
class medical_appointment(models.Model):
	...
	def onchange_name(self):
		...
		ins_record = ins_obj.sudo().search([('medical_insurance_partner_id', '=', self.patient_id.patient_id.id)])
		if len(ins_record)>=1:
	...
	def onchange_patient(self):
		if not self.inpatient_registration_id:
			self.patient_id = ""
		inpatient_obj = self.env['medical.inpatient.registration'].sudo().browse(self.inpatient_registration_id.id)
